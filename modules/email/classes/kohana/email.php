...
class Kohana_Email {
	...
	public static function mailer()
	{
		if ( ! Email::$_mailer)
		{
			...
			$config = Kohana::$config->load('email.'.Email::$_config) + array(
				'driver'  => 'native',
				'options' => array(),
			);
			...
			if ($driver === 'smtp')
			{
				...
				{
					// Set hostname
					$transport->setHost($options['hostname']);
				}
	 ...
	 * @return  Email
	 */
	public static function factory($subject = NULL, $message = NULL, $type = NULL, $config = 'default')
	{
		return new Email($subject, $message, $type, $config);
	}
	 ...
	 * @return  void
	 */
	public function __construct($subject = NULL, $message = NULL, $type = NULL, $config = 'default')
	{
        ...
        $config = Kohana::$config->load('email.'.Email::$_config);
        
        // Set name from
        if (isset($config['namefrom']))
            $name_from = $config['namefrom'];
        else
            $name_from = NULL;
        ...
        elseif (isset($config['options']['username']))
            $email_from = $config['options']['username'];
        else
            $email_from = 'mail@exemple.com';
        
        // Apply from
        $this->from($email_from, $name_from);
